jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_life_coach_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-life-coach-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-life-coach-plugin-install');
                button.addClass('button-primary vw-life-coach-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-life-coach-recommended-plugins .vw-life-coach-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-life-coach-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-life-coach-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-life-coach-recommended-plugins').css('display','none');
    });
